...
func (c *Category) FindCategoryBycourseID(course_id string) (Category, error) {
	var id, name, description string
	err := c.db.QueryRow(
		"SELECT cat.id, cat.name, cat.description FROM categories cat "+
			"join courses c ON cat.id = c.category_id WHERE c.id = $1;", course_id,
